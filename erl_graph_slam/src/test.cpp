...
void edgeCallback(const geometry_msgs::PoseWithCovarianceStamped::Ptr& msg)
{
	...
	*/
	//------------------------------ Optimization part ------------------------------//
	if(travel_dist_linear >= 0.7 || travel_dist_angular >= 0.4){ // TODO : should set by rosparam 0.7 0.4
...
/*
  if (n.getParam("history", history))
  {
    ROS_INFO("Got 'history' param: %d", history);
  }
  else
  {
    ROS_ERROR("Failed to get param 'history'");
  }
*/

  //set first pose node
  Pose2 priorMean(0.0, 0.0, 0.0); // prior at origin
  noiseModel::Diagonal::shared_ptr priorNoise = noiseModel::Diagonal::Sigmas(Vector3(0.01, 0.01, 0.001));
